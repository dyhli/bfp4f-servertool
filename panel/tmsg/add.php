...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['msg']) && isset($_POST['time']) && isset($_POST['active'])) {
	...
	if(count($errors) == 0) {
		...
		$result = $tmsg->createMessage(array(
		...
		if($result == 'OK') {
			...
			$log->insertActionLog($userInfo['user_id'], 'Timed message added');
		} else {
...
?>
			...
			<div class="row">
				<div class="col-md-8 col-md-offset-2">
					...
					<h2><i class="fa fa-plus green"></i> <?=$lang['tool_tmsg_add']?> <small><?=$lang['tool_tmsg']?></small></h2>
					<hr />
...
?>
									<option data-icon="fa fa-clock-o" value="<?=$time?>"><?=replace($lang['tool_tmsg_opt'], array('%s%' => $time))?></option>
<?php
}
?>
								</select>
						...
						<div class="form-group">
							<div class="col-sm-9 col-sm-offset-3">
								<div class="checkbox">
									<label><input type="checkbox" name="active" checked /> <span><?=$lang['tool_tmsg_help1']?></span></label>
								</div>
							</div>
 						</div>
